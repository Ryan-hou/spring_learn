...
public interface ApplicationContext extends BeanFactory {
     ...
     *3.ApplicationContext 以单例的模式，通过主动调用 getBean 实例化、注入属性、然后初始化BeanFactory中所有的Bean
     * 由于所有的 BeanPostProcessor 都已经在第 2 步中完成实例化了，因此接下来实例化的是普通 Bean，
     * 因此普通 Bean 的初始化过程可以正常执行。
     *4.调用 getBean 时，委托给 BeanFactory，此时只是简单的返回每个 Bean 单例，
     * 因为所有的 Bean 实例在第三步都已经生成了。
     */
}
